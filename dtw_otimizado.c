...
float dtw_compute(const float *signal_a, const float *signal_b, dtw_path_point_t *path_out, int *path_length) {
    ...
    /* ----------------------------------------------------------------------
     * FASE 1: TABULAÇÃO COM OTIMIZAÇÃO (Janelamento Sakoe-Chiba)
     * Complexidade reduzida de O(N^2) para O(N*w)
     * ---------------------------------------------------------------------- */

    // 1. Inicializa TODA a matriz com Infinito para blindar o backtracking
    for (i = 0; i < DTW_SIGNAL_SIZE; i++) {
        for (j = 0; j < DTW_SIGNAL_SIZE; j++) {
            matriz_custo[i][j] = DTW_INFINITY;
        }
    }
    ...
    // 3. Preenche as bordas iniciais respeitando o limite da Janela (w)
    for (i = 1; i <= DTW_WINDOW && i < DTW_SIGNAL_SIZE; i++) {
        matriz_custo[i][0] = abs((int)signal_a[i] - (int)signal_b[0]) + matriz_custo[i-1][0];
    }
    for (j = 1; j <= DTW_WINDOW && j < DTW_SIGNAL_SIZE; j++) {
        matriz_custo[0][j] = abs((int)signal_a[0] - (int)signal_b[j]) + matriz_custo[0][j-1];
    }

    // 4. O Miolo Otimizado (A Mágica acontece aqui)
    for (i = 1; i < DTW_SIGNAL_SIZE; i++) {
        // Ajusta dinamicamente onde o loop interno começa e termina
        int start_j = (i - DTW_WINDOW > 1) ? i - DTW_WINDOW : 1;
        int end_j = (i + DTW_WINDOW < DTW_SIGNAL_SIZE) ? i + DTW_WINDOW : DTW_SIGNAL_SIZE - 1;

        for (j = start_j; j <= end_j; j++) {
            custo_atual = abs((int)signal_a[i] - (int)signal_b[j]);

            matriz_custo[i][j] = custo_atual + min_of_3(
                matriz_custo[i-1][j],   // Custo cima
                matriz_custo[i][j-1],   // Custo esquerda
                matriz_custo[i-1][j-1]  // Custo diagonal
            );
        }
    }
